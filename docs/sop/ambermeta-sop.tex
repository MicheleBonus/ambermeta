...
\midrule
Replicas inside each stage & \code{equilibration/run1..run6} & Accept the proposal.\\
Numbered replicas in a stage & \code{prod/01..05} & Accept the proposal.\\
Replicas beside the stages & \code{run1..run4} next to \code{equilibration/} & Pick the top level in \textbf{Define replicas\ldots}; accept only \code{run1..run4}.\\
Replica id also in filenames & \code{prod/01/GB1\_204\_01\_000} & Pick the replica level manually; accept only the numbered members.\\
\bottomrule
\end{tabularx}
\end{center}
