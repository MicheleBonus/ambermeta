...
single YAML file describing the campaign, together with the summary artifacts of \S4.5.
...
output logs, AmberMeta will say so rather than guess. That is the point, but it does
...
AmberMeta expects the file kinds below, the topology once per system and the rest once
...
like a run, such as a lone \code{.in} in an analysis folder, becomes a step you then
...
members are already proposed, so accept them. Otherwise pick the directory level that
...
\begin{description}[leftmargin=1.4em,style=nextline,font=\bfseries]
  \item[\textsc{applied}] Inferences AmberMeta already acted on, each with an
        \textbf{Undo}. Skim these, because they are claims about your data. The starting
...
a finding left unresolved is an unanswered question travelling with your data.

\subsection{Save, then Plan}

Press \textbf{Save} to write \code{manifest.yaml} into the system directory. That is the
deposit artifact.

Then press \textbf{Plan}, and keep the files it writes. \textbf{This step is required,
not optional.} Plan writes \code{summary.json} and \code{methods\_summary.json} beside
the manifest: the manifest records what your files \emph{are}, and these record what
AmberMeta \emph{made of them}, run by run, with every number it derived.

That second record is what lets us audit the parser against real data. AmberMeta is being
developed against this collection, and the failures worth finding are the ones that occur
on layouts and AMBER versions nobody anticipated. Those are invisible in a manifest and
obvious in a summary: a duration that disagrees with the trajectory, a run reported as
unfinished that plainly finished, a continuity gap where the chain is sound. If a system's
summary files are not deposited, its quirks stay undiscovered until someone hits them in
an analysis.

So: deposit \code{manifest.yaml}, \code{summary.json} and \code{methods\_summary.json}
together, and report anything in them that contradicts what you know about the runs.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Plan} run, and \code{summary.json} and \code{methods\_summary.json} deposited with it.
\end{enumerate}
\end{note}
