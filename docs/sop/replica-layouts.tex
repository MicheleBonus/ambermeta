...
\begin{itemize}
  \item \textbf{\code{sys004} contains no output logs at all.} 5100 files --- 2496
        restarts, 2496 trajectories, 104 inputs --- and not one \code{.mdout}. A manifest
        can still be built, but nothing in it is confirmable: no achieved simulation
        time, no temperature, no completion status, and every junction between runs
        reported as an unexplained gap. This is the single most consequential gap found
        in the corpus.
...
\midrule
Replicas within stages   & \code{equilibration/run1..run6}, \code{production/run1..run8} & yes & ---\\
Numbered within stages   & \code{equil/01..05}, \code{prod/01..05}                        & yes & ---\\
...
two are \emph{declined}, not mis-grouped --- and in both the runs are still discovered
...
\begin{enumerate}
  ...
  \item \textbf{Treat missing output logs as a deposit blocker.} \code{sys004} is the
        case to cite: a 5100-file deposit from which nothing can be confirmed. A one-line
        check at deposit time (every run directory holding a \code{.mdout}) would have
        caught it.
  ...
  \item \textbf{Do not weaken the disjoint-families rule.} The \code{sys011} refusal is
        ...
        mode it prevents --- silently merging two experiments into one campaign --- is not.
\end{enumerate}
