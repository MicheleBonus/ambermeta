...
# ---------------------------------------------------------------------------
# The emitted key sets
# ---------------------------------------------------------------------------

def _stage(name: str, lineage: str = None, steps: float = 1000.0, dt: float = 0.002):
    """A stage carrying just enough for `totals()`, which reads only the mdin details.

    Synthetic rather than discovered: the fixture directory holds a 12 MB prmtop and five
    160 kB mdouts, and none of it is involved in the arithmetic being pinned here.
    """
    stage = SimulationStage(name=name, lineage=lineage)
    stage.mdin = SimpleNamespace(details=SimpleNamespace(length_steps=steps, dt=dt))
    return stage


def test_totals_emits_exactly_two_keys_for_an_untagged_protocol():
    """Pinned directly, not only through the golden.

    Nothing asserted `totals()`'s key set, so the summary.json golden was its only guard —
    and a golden can be regenerated by someone who did not realise a key had appeared.
    Both new keys in this PR are conditional on the document being multi-lineage; this is
    the assertion that says so about the branch nobody looks at.
    """
    protocol = SimulationProtocol(stages=[_stage("prod_0001"), _stage("prod_0002")])
    assert set(protocol.totals()) == {"steps", "time_ps"}
    assert set(protocol.to_dict()) == {"totals", "stages"}


def test_an_untagged_protocol_gains_neither_lineage_key():
    """The negative half of ruling 13.1.1, at the artifact rather than the inference level.

    `lineage_count` and `lineages` are what a reader of summary.json would use to decide
    whether a document declares members. Emitting either for a document that declares none
    is a claim, and it is the claim the untagged goldens exist to refuse.
    """
    protocol = SimulationProtocol(stages=[_stage("prod_0001"), _stage("prod_0002")])
